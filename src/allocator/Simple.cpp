...
void Simple::put_val(size_t offset, size_t value) { *reinterpret_cast<size_t *>((char *)_base + offset) = value; }

size_t Simple::get_val(size_t offset) const { return *(size_t *)((char *)_base + offset); }
    ...
    do {
        ...
        if (free_info.second >= N) {
            ...
            if (descr_offset.first == m_first_free_descr) {
                prev_free_offset = _base_len - size_t_size;
                free_offset = get_val(prev_free_offset);
                continue;
            }
            ...
            if (free_info.second - N >= 2 * size_t_size) {
                ...
                put_val(free_offset + N + size_t_size,
                        get_val(free_offset + 1 * size_t_size) - N); // new freespace size
...
void Simple::realloc(Pointer &p, size_t N) {
    ...
    if (descr_info.second >= N) // make it smaller
    {
        ...
        if (next_free_piece == end_of_part) // we're uniting two pices this + free
        {
            ...
            put_val(descr_info.first + N + size_t_size,
                    descr_info.second - N + get_val(next_free_piece + 1 * size_t_size)); // size
 ...
 * @param p Pointer
 */
void Simple::get_prev_next_free(size_t end_of_part, size_t &prev_free_piece, std::pair<size_t, size_t> descr_info,
...
void Simple::unit_free_picies(size_t end_of_part, size_t prev_free_piece, std::pair<size_t, size_t> descr_info,
                              size_t next_free_piece, bool from_realloc) {
    if (!from_realloc &&
        prev_free_piece + get_val(prev_free_piece) == descr_info.first) // we're uniting two pices free + this
    {
        if (next_free_piece == end_of_part) // free + this + free
        {
            put_val(prev_free_piece, get_val(next_free_piece));
            put_val(prev_free_piece + 1 * size_t_size, get_val(next_free_piece + 1 * size_t_size) +
                                                           get_val(prev_free_piece + 1 * size_t_size) +
                                                           descr_info.second);
        } else // free + this
        {
            put_val(prev_free_piece + 1 * size_t_size, get_val(prev_free_piece + 1 * size_t_size) + descr_info.second);
        }
...
size_t Simple::find_descriptor_for_memory(size_t point) {
    size_t first_descr = _base_len - get_val(m_descr_table_size) * 2 * size_t_size - 4 * size_t_size;
...
void Simple::defrag() {
    ...
    while (free_offset != m_first_free_block) {
        ...
        memmove((char *)_base + free_offset, (void *)descr_info.first, descr_info.second);

        put_val(descr, (size_t)((char *)_base + free_offset));

        size_t block_end = (size_t)((char *)descr_info.first - (size_t)_base) + descr_info.second;
